...
@section('content')
    <form action="{{ route('dashboard.roles.store') }}" method="POST" enctype="multipart/form-data" class="card shadow-sm">
                                                ...
                                                <div class="card-body d-flex flex-column pb-4">
                                                    @if (is_array($permission))
                                                        @foreach ($permission as $singlePermission)
                                                            <label class="form-check form-check-sm form-check-custom form-check-solid fs-7 text-muted mb-3">
                                                                <input name="permissions[{{ $singlePermission }}]" value="{{ $singlePermission }}" class="form-check-input h-15px w-15px permissionCheck cursor-pointer" type="checkbox" @if (old('permissions.' . $singlePermission)) checked @endif>
                                                    ...
                                                    @else
                                                        @foreach ($permissions['permissionMaps'] as $permissionMap)
                                                            <label class="form-check form-check-sm form-check-custom form-check-solid fs-7 text-muted mb-3">
                                                                <input name="permissions[{{ $permissionMap . ' ' . $permission }}]" value="{{ $permissionMap . ' ' . $permission }}" class="form-check-input h-15px w-15px permissionCheck cursor-pointer" type="checkbox" @if (old('permissions.' . $permissionMap . ' ' . $permission)) checked @endif>
